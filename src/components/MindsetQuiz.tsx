...
import { BookOpen, Rocket, Star, LightbulbIcon, Award, Mail } from "lucide-react";
...
const MindsetQuiz = () => {
  ...
  const handleEmailSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Calculate result first to get the quiz result
    const quizResult = calculateResultType(answers);
    
    try {
      // Send email to backend
      const response = await fetch('http://localhost:3001/api/emails/quiz', {
        method: 'POST',
        headers: {
      ...
      // Still show results even if email saving fails
      alert("There was an issue saving your email, but you can still see your results!");
    }
  ...
  const calculateResultType = (quizAnswers: Record<string, string>): ResultType => {
    let growthCount = 0;
    let fixedCount = 0;
  ...
  const calculateResult = (quizAnswers: Record<string, string>) => {
    const resultType = calculateResultType(quizAnswers);
  ...
  return (
    <>
      ...
      <div className="w-full max-w-3xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden mt-12 mb-16">
        <div className="p-8">
          <motion.div
          ...
          >
            <h2 className="text-3xl font-playfair font-bold text-navy mb-6">Power-Up Your Mind Quiz</h2>
            ...
            <Button
              onClick={startQuiz}
              className="bg-navy hover:bg-light-navy text-white px-10 py-5 text-xl rounded-lg shadow-md hover:shadow-lg transition-shadow"
            >
              Take Quiz
            </Button>
          </motion.div>
              ...
              >
                <div className="mb-8">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                    <div
                      className="bg-gradient-to-r from-yellow-400 to-gold h-2.5 rounded-full transition-all duration-300"
                  ...
                  <form onSubmit={form.handleSubmit(handleNext)} className="space-y-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="space-y-3">
                          <FormLabel className="text-xl font-semibold text-navy text-center block">
                    ...
                    <div className="pt-6 flex justify-end">
                      <Button
                        ...
                        className="bg-navy hover:bg-light-navy text-white px-8 py-3 text-lg rounded-md shadow hover:shadow-md transition-shadow disabled:opacity-50"
                      >
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-4 max-w-sm mx-auto">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      required
                      className="pl-10 pr-4 py-3 text-base border-gray-300 focus:border-navy focus:ring-navy rounded-md shadow-sm"
                    />
                  </div>
                  <Button
                    type="submit"
                    className="w-full bg-gold hover:bg-yellow-500 text-navy px-8 py-3 text-lg rounded-md shadow hover:shadow-md transition-shadow"
                  >
                    Unlock My Results!
                  </Button>
      ...
      {/* Results Dialog (separate from quiz modal) */}
